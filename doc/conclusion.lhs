...
\end{itemize}
In the state-of-the-art, there's a trade-off between having easy-to-write session types and idiomatic programs, which is largely due to the trouble of encoding static linearity checking in an unrestricted language~\cite{orchardyoshida17}.

We side-step this trade-off by relying on Linear Haskell to check linearity, and our implementation supports all relevant features, including multiple channels, delegation, recursion, and highly idiomatic code.
